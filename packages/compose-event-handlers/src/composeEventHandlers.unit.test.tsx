...
function getEvent(): Event { return new Event("test", { cancelable: true }) };
...
describe("composeEventHandlers", () => {
  ...
  test('Compose 2 valid handlers', async () => {
    const logSpy = jest.spyOn(console, 'log');
    const handler = composeEventHandlers((e) => console.log("first"), (e) => console.log("second"));
    handler(getEvent());
    expect(logSpy).toHaveBeenCalledWith('first');
  ...
  test('Compose 3 valid handlers', async () => {
    const logSpy = jest.spyOn(console, 'log');
    const handler = composeEventHandlers((e) => console.log("first"), (e) => console.log("second"), (e) => console.log("third"));
    handler(getEvent());
    expect(logSpy).toHaveBeenCalledWith('first');
    expect(logSpy).toHaveBeenCalledWith('second');
