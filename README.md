...
- `packages/@clutchd/ui`: Core React components
- `packages/@clutchd/utils`: Shared React utilities
- `packages/@clutchd/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset-clutchd`: ESLint preset
...
For `clutchd-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `clutchd-ui` then instructs the consumer to select the correct format:

```json:clutchd-ui/package.json
{
  "name": "@clutchd/ui",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `clutchd-ui/dist` which contains the compiled output.
...
Each file inside of `clutchd-ui/src` is a component inside our design system. For example:

```tsx:clutchd-ui/src/Button.tsx
...
- Support using module path aliases like `@clutchd-ui` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@clutchd-ui/src';
...
Turborepo runs the `build` script for all publishable packages (excluding docs) and publishes the packages to npm. By default, this example includes `clutchd` as the npm organization. To change this, do the following:

- Rename folders in `packages/*` to replace `clutchd` with your desired scope
- Search and replace `clutchd` with your desired scope
- Re-run `yarn install`
